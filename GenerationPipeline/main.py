...
from GenerationPipeline.llm_prompt import generate_prompt
from GenerationPipeline.image_sampling import sample_from_folder, sample_from_cluster
from GenerationPipeline.create_dataset import image_to_base64
from GenerationPipeline.create_dataset import save_to_json_file
...
def generate_with_retries(model_name, sampled_images, max_retries=7):
    """
    Generates a dialogue prompt using the Gemini API with retry mechanism.

    This function attempts to generate a dialogue prompt using the Gemini API, retrying
    with exponential backoff if the quota is exceeded.
